...
from Qwen2VL_DART_ViT.FiCoCo.FiCoCo_V_no_CLS import *
#from Qwen2VL_DART_ViT.FiCoCo.FiCoCo_L_no_CLS import *
...
#         "r": r,
#         "size": None,
#         "class_token": False, #True is not None,
#     }
...
class PatchMerger(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return x # 特征维度从embed_dim -> hidden_dim 
...
class Qwen2VLVisionBlock(nn.Module):
    ...
    def forward(self, hidden_states_pkg : dict, cu_seqlens, rotary_pos_emb, prune=False, merge_visual=False,grid_thw=None, r=None) -> torch.Tensor:
        ...
        remain_indices = None
        if merge_visual and prune and r is not None:
            #print("hidden_states shape 1",hidden_states.shape)
            hidden_states, attn_weights, metric, att_score = adapt_qwen2vl_output(hidden_states, output)
            ...
            if rr > 0:
                ...
                order_indices, merge_indices, remain_indices, att_prob = Filter(
                    att_score, metric, rr, include_class_token=False, grid_thw=grid_thw)                     # qwen没有class token   _ficoco_info["class_token"]
                # Correlate 
                target_indices, top_values = Correlate(merge_indices, att_prob)
                
                #print("order_indices",order_indices.shape, merge_indices.shape, remain_indices.shape, att_prob.shape,target_indices.shape, top_values.shape,hidden_states.shape)
                # order_indices torch.Size([1, 5221, 1]) torch.Size([1, 42, 1]) torch.Size([1, 5179, 1]) torch.Size([1, 5220, 5220]) torch.Size([1, 42, 11]) torch.Size([1, 5220, 11]) torch.Size([1, 5220, 1280])
                #print(order_indices,merge_indices, remain_indices,target_indices,top_values)
                # Compress
                #remain_indices, _ = torch.sort(remain_indices, dim=-1)

                hidden_states, _ = merge_ficoco_v(
                    Compress, hidden_states, merge_indices, remain_indices, top_values, target_indices, rr, None) #  _ficoco_info["size"]
                #print("hidden_states after block",hidden_states.shape, "remain", remain_indices.shape)
                
                remain_indices = remain_indices[0,:,0].sort().values # remain torch.Size([ 5178])
                #print("remain",remain_indices)
                # hidden_states = hidden_states[:,remain_indices,:]
                ...
                for b in range(frame_counts.shape[0]):
                    ...
                    keep_count = ((remain_indices >= start) & (remain_indices < end)).sum()
...
def adapt_qwen2vl_output(hidden_states, output, bsz=1):
    """
    将 Qwen2VL 的注意力输出字典转换为 FiCoCo 接口格式
    """
    attn_output = hidden_states #不能output['attn_output']  # (seq_length, embed_dim)

    # reshape 成 (bsz, seq_length, embed_dim)
    attn_output = attn_output.unsqueeze(0) if attn_output.dim() == 2 else attn_output

    # attn_scores: (seq_length, seq_length) → (bsz, num_heads, seq_length, seq_length)
    attn_weights_reshaped = output['attn_scores'].unsqueeze(0).unsqueeze(0)

    # mean_keys: (seq_length, num_heads, head_dim) → 在 head 上取平均
    #print("k states",output['k_states'].shape)
    mean_keys = output['k_states'].mean(dim=0)  # (seq_length, head_dim)
    mean_keys = mean_keys.unsqueeze(0)  # (bsz, seq_length, head_dim)
    #print("mean_keys",mean_keys.shape)

    # att_score: 对 attn_scores 在 batch/head 上取平均，这里只有一个 → (seq_length, seq_length)
    #print("attn score",output['attn_scores'].shape)
    att_score = output['attn_scores'].mean(dim=0)
...
class Qwen2VLDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        '''
        print("causal_msk",hidden_states.shape,attention_mask, past_key_value,position_ids.shape)
        if past_key_value.key_cache == []:
            print("no cache")
        else:
            print("cache",past_key_value.key_cache[0].shape)#,past_key_value.key_cache[0][0][0])
            print(len(past_key_value.key_cache))
        '''
        # causal_msk torch.Size([1, 1549, 3584]) None DynamicCache() torch.Size([3, 1, 1549])
        #no cache
        #causal_msk torch.Size([1, 1549, 3584]) None DynamicCache() torch.Size([3, 1, 1549])
        #cache torch.Size([1, 4, 1549, 128])  # “4” 就是模型里 multi-head attention 的头数
        # 1
        # causal_msk torch.Size([1, 1, 3584]) None DynamicCache() torch.Size([3, 1, 1])
        # cache torch.Size([1, 4, 1549, 128])
        #causal_msk torch.Size([1, 1, 3584]) None DynamicCache() torch.Size([3, 1, 1])
        ...
        #print("llm layer key states",hidden_states.shape, self_attn_weights.shape, key_states.shape)
        if self_attn_weights is not None:
            print(self_attn_weights.shape)
        remain_indices = None
        if prune and r is not None:
            #print("hidden_states shape 1",hidden_states.shape)
            #hidden_states, attn_weights, metric, att_score = adapt_qwen2vl_output_llm(hidden_states, self_attn_weights, present_key_value, query_states, key_states, value_states)
            hidden_states = hidden_states.unsqeeze(0)
            #print("hidden_states shape 2",hidden_states.shape)
            rr = r

            v_token = img_len
            metric1 = metric[:, :v_token]
            tem = hidden_states[:,img_start:] # 应为[batch_size, seq_len, hidden_dim]
            hidden_states_sys = hidden_states[:,:img_start]
            hidden_states_pre = tem[:, :v_token]
            hidden_states_post = tem[:, v_token:]

            att_score = self_attn_weights[:,img_start:,img_start:] # 应为[batch_size, seq_len, seq_len]

            if rr > 0:
                # Filtering and merging
                merge_indices, remain_indices, att_score_list = Filter_l(
                    tem, att_score, v_token, rr, 
                    False, False  #_ficoco_info["class_token"], _ficoco_info["distill_token"]
                )
                target_indice, top_values = Correlate_l(
                    merge_indices, att_score_list, att_score, rr
                )
                hidden_states_pre, _ficoco_info["size"] = merge_ficoco_l(
                    Compress_l, hidden_states_pre, merge_indices, remain_indices, 
                    target_indice, top_values, rr, _ficoco_info["size"] 
                )
                hidden_states = torch.cat([hidden_states_sys, hidden_states_pre, hidden_states_post], dim=1)

                #
                print("hidden_states after block",hidden_states.shape, "remain", remain_indices.shape)
                hidden_states = hidden_states[0]
                remain_indices = remain_indices[0,:,0] # remain torch.Size([ 5178, 1])
                print("remain",remain_indices)
...
def adapt_qwen2vl_output_llm(hidden_states, attn_weights, present_key_value, query_states, key_states, value_states):
    """
    输入：Qwen2VL attn.forward 的输出
    输出：hidden_states, attn_weights, metric, att_score
    """
    # metric 用 key_states
    metric = key_states
    
    # att_score 用 attn_weights
    att_score = attn_weights
    
    return hidden_states, attn_weights, metric, att_score
...
class Qwen2VisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        cu_seqlens = F.pad(cu_seqlens, (1, 0), value=0) # [2] 序列边缘，起始和结尾
...
class DART(Qwen2VLModel):
    ...
    def forward(
        ...
        target_keep_ratio = 1.0 - self.config.DART_config['reduction_ratio']  # 0.2
        ...
        if self.config.DART_config is not None and self.config.DART_config['Sparse']\
            ...
            K = DART_config['pruned_layer']
            # prune_layers = [i for i in range(len(self.blocks)) if i > control_encoding_layer]
            prune_layers = [i for i in range(K-1,K-1+total_prune_layer)]
            
            self.update_layer(device,dtype,prune_layers)
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                #print("DART_config",self.config,self.config.DART_config)
                ...
                if DART_config is not None and DART_config['Sparse']:
                    ...
                    if decoder_layer.self_attn.layer_idx >= K and image_token_length > 1 and\
                        ...
                        desired_keep_ratio = max(target_keep_ratio / current_keep_ratio, min_layer_keep)

                        # 本层要保留的 token 数
                        keep_tokens = int(image_token_length * desired_keep_ratio)
                        # 本层要合并的 token 对数
                        tome_r = image_token_length - keep_tokens
                        tome_r = (tome_r // 4) * 4  # 只能取下限


                        #hidden_states_pkg['remain_indices']
                        layer_outputs = decoder_layer(
                            hidden_states,
                            attention_mask=causal_mask,
                            position_ids=position_ids,
                            ...
                            prune=prune,
                            r=tome_r,
                            img_start=image_token_start_index,
                            img_len =  image_token_length
                        )
                        ...
                        hidden_states = layer_outputs['hidden_states']
                        retained_image_tokens_index = layer_outputs['remain_indices']
    ...
    def get_retained_image_token_random(self, config: Qwen2VLConfig, last_layer_state: torch.Tensor, any_states: torch.Tensor) -> torch.Tensor:
        DART_config = config.DART_config
        reduction_ratio = DART_config['reduction_ratio']
        image_token_start_index = DART_config['image_token_start_index']
        image_token_length = DART_config['image_token_length']
    ...
    def update_layer(self, device=None, dtype=None,layer_nums=[]):
        for k in layer_nums:
            # 获取需要替换的层索引
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel):
    ...
    def forward(
        ...
        ```"""
        #print("CUDA memory at the beginning of forward funcion :", torch.cuda.memory_allocated() / 1024**2, "MB")
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
        ...
        if inputs_embeds is None:
            ...
            if pixel_values_videos is not None:
                ...
                if self.config.DART_config is not None and self.config.DART_config['vit_Sparse']:
                    ...
                    #print("videmb",video_embeds.shape,retained_nums )
                    input_ids_new, retained_indices = _update_ids(input_ids, self.config.video_token_id, retained_nums)
                    #print("id new",input_ids_new.shape, retained_indices.shape)
                    #print(input_ids_new, retained_indices)
...
class DART_ViT(Qwen2VisionTransformerPretrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        if self.config.DART_config is not None and self.config.DART_config['vit_Sparse']\
            ...
            # prune_layers = [i for i in range(len(self.blocks)) if i > control_encoding_layer]
            prune_layers = [i for i in range(K,K+total_prune_layer)]
            #print("prune_layers",prune_layers)
            self.update_vision_block(device,dtype,prune_layers)
            self.update_attention_layer=True
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if DART_config is not None and DART_config['vit_Sparse']:
                ...
                if blk.layer_idx >= K and hidden_states_pkg['hidden_states'].shape[0]>1 and\
                    ...
                    hidden_states_pkg = blk(hidden_states_pkg, cu_seqlens=cu_seqlens, rotary_pos_emb=rotary_pos_emb, prune=prune, merge_visual=merge_visual,grid_thw=grid_thw, r=tome_r)
                    #hidden_states_pkg['remain_indices']

                    # 更新当前保留比例
                    current_keep_ratio *= desired_keep_ratio
                    # 更新位置编码
                    #print("rotary",rotary_pos_emb.shape,cu_seqlens,frame_counts)
                    rotary_pos_emb = rotary_pos_emb[hidden_states_pkg['remain_indices'],:]
                    ...
                    prune = False
                    hidden_states_pkg = blk(hidden_states_pkg, cu_seqlens=cu_seqlens, rotary_pos_emb=rotary_pos_emb, prune=prune, merge_visual=merge_visual,grid_thw=grid_thw)
            else:
                prune = False
                hidden_states_pkg = blk(hidden_states_pkg, cu_seqlens=cu_seqlens, rotary_pos_emb=rotary_pos_emb, prune=prune, merge_visual=merge_visual,grid_thw=grid_thw)
        ...
        # ------------------------END------------------------------------------
            ...
            #     #print(self._sparse_vit_saved )
            #     #print(hidden_states_pkg['hidden_states'].shape, cu_seqlens_pruned, rotary_pos_emb_pruned.shape)
            #     hidden_states_pkg = blk(hidden_states_pkg, cu_seqlens=cu_seqlens_pruned, rotary_pos_emb=rotary_pos_emb_pruned, prune=prune, merge_visual=merge_visual,grid_thw=grid_thw)
            # else:
    ...
    def update_vision_block(self, device, dtype, layer_nums):
...
def plot_layer_states( states: np.ndarray,
                    out_dir: str,
                    name: str,
                    cmap: str = 'rainbow' ):
    """
    ...
    plt.scatter(coords[:, 0], coords[:, 1], c=np.arange(len(states)), cmap=cmap)
...
class Qwen2RMSNorm_no_param(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        RMSNorm without learnable parameters (i.e., no gamma scaling).
        Normalizes hidden states to have unit variance (approximately).
        """
        super().__init__()
        self.variance_epsilon = eps
