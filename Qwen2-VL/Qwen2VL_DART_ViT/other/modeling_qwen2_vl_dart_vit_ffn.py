'''
剪掉的token不经过ffn层，只经过attention并更新，在vit最后一层不丢弃
'''
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        # ---------可视化------------
            random_vectors.append(
                hidden_states_pkg['hidden_states'][rand_idx]
                .detach()
                .clone()
                .to(torch.float32)    # ← cast here
                .cpu()
                .numpy()
            )

            mean_vectors.append(
                hidden_states_pkg['hidden_states']
                .mean(dim=0)
                .detach()
                .clone()
                .to(torch.float32)    # ← and here
                .cpu()
                .numpy()
            )
        random_vectors = np.stack(random_vectors, axis=0)
        mean_vectors   = np.stack(mean_vectors,   axis=0)
        output_folder = "./layer_plots"

        # 随机点的 PCA 可视化
        plot_layer_states(
            states = random_vectors,
        ...
        #画在同一个图
        plot_multi_states(
            {'random': random_vectors, 'mean': mean_vectors},
            out_dir = output_folder,
            name   = "compare_random_mean"
        )
...
def plot_layer_states( states: np.ndarray,
    ...
    """
    print("states",states.shape,states[29],states[30],states[31])
    states = states[:-1]
    # 1. 确保输出文件夹存在
...
def plot_multi_states(states_dict: Dict[str, np.ndarray], out_dir: str, name: str):
    """
    states_dict: {'random': array(L,D), 'mean': array(L,D), ...}
    """
    # 1. 统一截尾
    trimmed = {k: v[:-1] for k,v in states_dict.items()}
    # 2. 合并、PCA
    all_states = np.vstack(list(trimmed.values()))
    pca = PCA(n_components=2)
    coords = pca.fit_transform(all_states)

    # 3. 根据每组长度分割 coords
    splits = np.cumsum([v.shape[0] for v in trimmed.values()])[:-1]
    groups = np.split(coords, splits, axis=0)

    # 4. 绘图
    plt.figure(figsize=(6,6))
    for (label, _), grp in zip(trimmed.items(), groups):
        plt.scatter(grp[:,0], grp[:,1], label=label)
        for i,(x,y) in enumerate(grp):
            plt.text(x, y, str(i), fontsize=7, alpha=0.6)
    plt.title(f"PCA compare ({name})")
