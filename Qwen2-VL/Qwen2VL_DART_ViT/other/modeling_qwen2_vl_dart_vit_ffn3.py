'''
剪掉的token不经过ffn层，只经过attention但不更新，在vit最后一层丢弃掉
'''
...
class Qwen2VLVisionBlock(nn.Module):
    ...
    def forward(self, hidden_states_pkg: dict, cu_seqlens, rotary_pos_emb):
        ...
        skip_ffn_mask = hidden_states_pkg.get('skip_ffn_mask', None)
        ...
        attn_output = attn_out['attn_output']
        if skip_ffn_mask is not None:
            attn_output = attn_output.clone()
            attn_output[skip_ffn_mask] = 0.0
        hidden_states = hidden_states + attn_output

        # 3) norm2：保留全部 token 的 hidden，用于后面 MLP，但只在 kept token 上 normalize
        normed2 = self.norm2(hidden_states)
        if skip_ffn_mask is not None:
            normed2 = normed2.clone()
            normed2[skip_ffn_mask] = hidden_states[skip_ffn_mask]

        # 4) 只对 kept token 做 MLP
        if skip_ffn_mask is not None:
            keep_idxs = torch.nonzero(~skip_ffn_mask, as_tuple=True)[0]
            kept = self.mlp(normed2[keep_idxs])
            mlp_out = torch.zeros_like(normed2)
            mlp_out[keep_idxs] = kept
        else:
        ...
        return {
            ...
            'k_states': attn_out['k_states'],
            'attn_scores': attn_out['attn_scores'],
...
class DART_ViT(Qwen2VisionTransformerPretrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        keep_idxs = torch.nonzero(~hidden_states_pkg['skip_ffn_mask'], as_tuple=True)[0]   # 只保留 skip_ffn_mask=False 的 token

        # 用索引切掉 skip 的部分
        hidden_states_pkg['hidden_states'] = hidden_states_pkg['hidden_states'][keep_idxs]
        if frame_counts !=0:
            frame_counts = frame_counts/(self.config.spatial_merge_size**2)
            #print("frame2",frame_counts)tensor([1044.], device='cuda:0')
