...
# export HIPRUNE_QWEN_RETENTION=0.223
...
class VisionAttention(nn.Module):
    ...
    def forward(
        ...
        #print("CUDA memory allocated before attention calculation2:", torch.cuda.memory_allocated() / 1024**2, "MB") # DEBUG
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                # vision数量不变
                image_embeds, _, attn_list = self.visual(pixel_values, grid_thw=image_grid_thw)
            ...
            if pixel_values_videos is not None:
                ...
                # vision数量不变
                video_embeds, _, attn_list = self.visual(pixel_values_videos, grid_thw=video_grid_thw)
        ...
        if inputs_embeds.shape[1] != 1: # KV cache
            #print(self.config.DART_config)
            
            RETAIN=1-self.config.DART_config['reduction_ratio'] # ratio of tokens to be retained
            
            #print("retain",RETAIN)
            #print("attn_list",attn_list)
            shallow_attention = attn_list[OBJECT_LAYER - 1]
            deep_attention = attn_list[-1]
            #print("shape",shallow_attention.shape,deep_attention.shape)

            # shallow_attention = shallow_attention.mean(dim=0).mean(dim=0)#[:, 1:] # Global attention
            # deep_attention = deep_attention.mean(dim=0).mean(dim=0)#[:, 1:] # Global attention
            # # 假设 shallow_attention 和 deep_attention 都是 (5220,)
            # shallow_attention = shallow_attention.view(-1, 4).mean(dim=1)  # (1305,)
            # deep_attention = deep_attention.view(-1, 4).mean(dim=1)        # (1305,)
    ...
    def get_retained_image_token_Hiprune_llava(self, image_features, all_image_attentions, reduction_ratio): 
        # (B, N, C), (B, M, N) = (1, 576, 1024), (1, 16, 576)
        # _ , (B, H, N, N) ?
        HIPRUNE_RETENTION=reduction_ratio # ratio of tokens to be retained
        HIPRUNE_ALPHA=0.1
        HIPRUNE_OBJECT_LAYER=14

        # assert images.shape[0] == 1, "Only support for batch_size = 1"
         
        B, N = image_features.shape[:2]
        visual_token_num = int(HIPRUNE_RETENTION * N)
        # image_attentions = image_attentions.max(dim=1)[0] # (B, N) = (1, 576)
        deep_attention = all_image_attentions[-1].cuda() # all_image_attentions[self.config.mm_vision_select_layer].cuda()
        # deep_attention = deep_attention.mean(dim=1)[:, 0, 1:] # [cls] attention
        deep_attention = deep_attention.mean(dim=1).mean(dim=1) # [:, 1:] # Global attention

        shallow_attention = all_image_attentions[HIPRUNE_OBJECT_LAYER - 1].cuda()
        # shallow_attention = shallow_attention.mean(dim=1)[:, 0, 1:] # [cls] attention
        shallow_attention = shallow_attention.mean(dim=1).mean(dim=1) #[:, 1:] # Global attention
        shallow_token_num = round(visual_token_num * float(HIPRUNE_ALPHA) / 5)
        ...
        for i in range(B):
            ...
            shallow_token_indices = torch.cat([shallow_token_indices,
                                            ...
                                            shallow_token_indices + 24], dim=1) # (B, T * 5)
            shallow_token_indices = shallow_token_indices.clamp(0, N-1)
            ...
            available_index_mask = torch.zeros(1, N, dtype=torch.bool, device=image_features.device) # (B, N)
            available_index_mask.scatter_(1, shallow_token_indices, 1)
            
            deep_attention[i] -= available_index_mask.int().squeeze()
            
            deep_token_indices = torch.topk(deep_attention[i].unsqueeze(0), k=deep_token_num, dim=1)[1] # (B, T)
            token_indices = torch.cat([shallow_token_indices, deep_token_indices], dim=1)
            token_indices = torch.sort(token_indices, dim=1)[0] # (B, T)

            # generate index mask
            index_mask = torch.zeros(1, N, dtype=torch.bool, device=image_features.device) # (B, N)
    ...
    def get_retained_image_token_Hiprune(self, image_features, attn_list, reduction_ratio,image_grid_thw,input_ids): 
        HIPRUNE_RETENTION=reduction_ratio # ratio of tokens to be retained
        HIPRUNE_ALPHA=0.1
        HIPRUNE_OBJECT_LAYER=14
        B, N = image_features.shape[:2]

        shallow_attention = attn_list[HIPRUNE_OBJECT_LAYER - 1]
        deep_attention = attn_list[-1]
        
        select_mask = torch.zeros_like(deep_attention, dtype=torch.bool)
        visual_token_num = round(N * HIPRUNE_RETENTION)
        shallow_token_num = round((visual_token_num * HIPRUNE_ALPHA) / 5)
        ...
        deep_token_num = visual_token_num - shallow_token_indices.shape[0]
        available_index_mask = torch.zeros(N, dtype=torch.bool, device=input_ids.device)
        available_index_mask.scatter_(0, shallow_token_indices, 1)
...
class DART_ViT(Qwen2VisionTransformerPretrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        if self.config.DART_config is not None and not self.update_attention_layer:
            self.update_vision_block(device,dtype, [OBJECT_LAYER-1,len(self.blocks)-1])
            self.update_attention_layer=True
        ...
        for i, blk in enumerate(self.blocks):
            ...
            attn_weights = hidden_states_pkg['attn_scores']
            # with torch.no_grad():
            ...
            with torch.no_grad():
                if attn_weights is not None:
                    attn_weights = attn_weights.mean(dim=0).mean(dim=0)#[:, 1:] # Global attention
                    attn_weights = attn_weights.view(-1, 4).mean(dim=1)
        ...
        hidden_states = self.merger(hidden_states_pkg['hidden_states'])
        # hidden_states = hidden_states[reverse_indices, :]
        
        return hidden_states, frame_counts, attn_list
    ...
    def update_vision_block(self, device, dtype,layers):
        # 获取需要替换的层索引
        for k in layers:
            if k<0 : return 
            # 旧 block
