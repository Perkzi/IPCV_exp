    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                image_embeds, retained_nums = self.visual(pixel_values, grid_thw=image_grid_thw)
                if self.config.DART_config is not None and self.config.DART_config['vit_Sparse'] and retained_nums is not None:
                    # vision数量变化
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if DART_config is not None and DART_config['vit_Sparse']:
                ...
                if blk.layer_idx == K and hidden_states_pkg['hidden_states'].shape[0]>1:
                    ...
                    b_idx, a_unm_idx, a_src_idx, max_indices = self.vlm_sito_prune(
                        self.config, last_layer_state)
                    #print("b_idx, a_unm_idx, a_src_idx, max_indices",b_idx.shape, a_unm_idx.shape, a_src_idx.shape, max_indices.shape)
                    
                    # 保留的索引 = 基准 + 未剪枝 
                    retained_image_tokens_index = torch.cat([b_idx, a_unm_idx]).sort().values
        ...
        # ------------------------END------------------------------------------
            ...
            if blk.layer_idx == 32-1 and hasattr(self, "_sparse_vit_saved"):
                if recover:
                    hidden_states_pkg['hidden_states'] = self.vlm_sito_recover(hidden_states_pkg['hidden_states'],
                                                                            self._sparse_vit_saved['b_idx'],
                                                                            self._sparse_vit_saved['a_unm_idx'],
                                                                            self._sparse_vit_saved['a_src_idx'],
                                                                            self._sparse_vit_saved['max_indices'],
                                                                            self._sparse_vit_saved['retained_image_tokens_index'],
                                                                            )
                    #print("recover",hidden_states_pkg['hidden_states'].shape )
                del self._sparse_vit_saved
                # TODO: 恢复逻辑 修改framecounts
        ...
        if recover:
            return self.merger(hidden_states_pkg['hidden_states']), None
        return self.merger(hidden_states_pkg['hidden_states']), frame_counts



    def vlm_sito_prune(self, config, hidden_states: torch.Tensor, 
                    noise_alpha: float = 0.1, sim_beta: float = 1.0):
        """
        hidden_states: [seq_len, hidden_dim] 每个patch一个token
        reduction_ratio: 剪枝比例 (0~1)
        返回: b_idx, a_unm_idx, a_src_idx, max_indices
        """
        DART_config = config.DART_config
        ...
        # 2. 相似度矩阵
        cos_sim = norm_states @ norm_states.T   # [seq_len, seq_len]

        # 3. 相似度分数 + 噪声
        sim_score = cos_sim.sum(dim=-1)
        noise = torch.randn_like(sim_score) * noise_alpha
        sim_score = sim_beta * sim_score + noise

        # 4. 选基准 token (这里简单选 top-k，未来可改成均匀采样)
        num_base = int(seq_len * min(0.1,1-reduction_ratio))  # 保留10%作为基准，可调
        b_idx = sim_score.topk(num_base).indices.sort().values  # 正序

        # # 5. 对剩余 token 分类
        # remaining_idx = torch.tensor([i for i in range(seq_len) if i not in b_idx.tolist()],
        #                             device=device)
        # ---------更快的版本------------
        # 创建一个全 True 的掩码
        ...
        # 直接通过掩码提取剩余索引
        remaining_idx = torch.nonzero(mask).squeeze()
         # ---------更快的版本 end------------

        # 计算每个剩余 token 与基准 token 的最大相似度
        sim_to_base = cos_sim[remaining_idx][:, b_idx]
        max_sim_value, max_indices = sim_to_base.max(dim=1)

        # 按相似度排序，选出要剪掉的 token
        num_prune = int(seq_len * reduction_ratio) // 4 * 4 ## 向下取4的倍数


        sorted_idx = max_sim_value.argsort(descending=True)

        a_src_idx = remaining_idx[sorted_idx[:num_prune]].sort().values  # 剪掉的
        a_unm_idx = remaining_idx[sorted_idx[num_prune:]].sort().values  # 保留的

        return b_idx, a_unm_idx, a_src_idx, max_indices

    
    def vlm_sito_recover(self, pruned_states: torch.Tensor,
                     b_idx: torch.Tensor, a_unm_idx: torch.Tensor,
                     a_src_idx: torch.Tensor, max_indices: torch.Tensor,retained_image_tokens_index):
        """
        pruned_states: [len(b_idx)+len(a_unm_idx), hidden_dim]
        b_idx, a_unm_idx, a_src_idx: 三组索引 (正序)
        max_indices: 每个剪掉token对应的基准token在 b_idx 中的相对索引
        返回: [seq_len, hidden_dim] 恢复后的完整序列
        """
        seq_len = b_idx.shape[0] + a_unm_idx.shape[0] + a_src_idx.shape[0]
        hidden_dim = pruned_states.shape[-1]
        device = pruned_states.device

        out = torch.zeros(seq_len, hidden_dim, device=device, dtype=pruned_states.dtype)

        # 先把保留的 token 放回原位
        out[b_idx] = pruned_states[torch.searchsorted(retained_image_tokens_index, b_idx)]
        out[a_unm_idx] = pruned_states[torch.searchsorted(retained_image_tokens_index, a_unm_idx)]

        # 剪掉的 token 用对应的基准 token替代

        
        # src_base = b_idx[max_indices]  # 找到对应的基准索引
        # print("recover",a_src_idx.shape,src_base.shape)
        # out[a_src_idx] = out[src_base]

        # 计算每个剩余 token对应的基准 token
        # 注意：max_indices 的顺序和 remaining_idx 一一对应
        remaining_idx = torch.cat([a_unm_idx, a_src_idx])  # 剪枝时的顺序
        # 找出 a_src_idx 在 remaining_idx 中的位置
        src_positions = torch.searchsorted(remaining_idx, a_src_idx)

        # 只取出 a_src_idx 对应的基准 token
        src_base = b_idx[max_indices[src_positions]]
