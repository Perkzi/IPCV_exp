...
def bipartite_soft_fusion(metric, x, r, mode="mean", method="average"):
    """
    Token Fusion 版 bipartite soft matching
    method: "pruned" | "average" | "mlerp"
    """
    B, T, C = x.shape
    r = min(r, T // 2)
    if r <= 0:
    ...
    unm_pos = idx_a.gather(dim=-1, index=unm_idx.squeeze(-1))
    dst_pos = idx_b
...
class Qwen2VLVisionBlock(nn.Module):
    ...
    def forward(self, hidden_states_pkg: dict, cu_seqlens, rotary_pos_emb, tome_r=0, method="average"):
        ...
        if tome_r > 0:
            ...
            hidden_states, merged_idx = bipartite_soft_fusion(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values_videos is not None:
                ...
                if self.config.DART_config is not None and self.config.DART_config['vit_Sparse']:
                    ...
                    #print("video_emb",video_embeds.shape)
...
class DART_ViT(Qwen2VisionTransformerPretrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if DART_config is not None and DART_config['vit_Sparse']:
                ...
                if blk.layer_idx >= K and hidden_states_pkg['hidden_states'].shape[0]>1 and\
                    ...
                    if blk.layer_idx - K  < total_prune_layer/2:
                        method = "pruned"
                    else:
                        method = "average"
                    # 调用 block
        ...
        #print("hidden",hidden_states_pkg['hidden_states'].shape,frame_counts)
...
def _update_ids(input_ids, image_token_id, retained_nums):
    ...
    #print("total_retain",total_retain)
