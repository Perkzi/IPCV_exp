...
class Qwen2VLVisionBlock_Saint(Qwen2VLVisionBlock):
    def __init__(self, config, layer_idx,attn_implementation: str = "sdpa") -> None:
        super().__init__(config, layer_idx,attn_implementation)
        self.config = config

    def forward(self, hidden_states_pkg : dict, cu_seqlens, rotary_pos_emb, prune=False) -> torch.Tensor:
        hidden_states = hidden_states_pkg['hidden_states']
        output = self.attn(
            self.norm1(hidden_states), cu_seqlens=cu_seqlens, rotary_pos_emb=rotary_pos_emb
        )
        ...
        if prune:
            remain_idx = self.iterative_drop_full_graph(self.config, hidden_states, output['k_states'].mean(1)) #  torch.Size([5220, 1280]) torch.Size([5220, 16, 80])
            hidden_states = hidden_states[remain_idx]
        else:
            remain_idx = None
    ...
    def iterative_drop_full_graph(self, config, last_layer_state: torch.Tensor, key_state: torch.Tensor):
        # print("last_layer_state.shape",last_layer_states.shape,key_states.shape)
        # last_layer_state = last_layer_states[0]
        # key_state = key_states[0]

        DART_config = config.DART_config
        reduction_ratio = DART_config['vit_reduction_ratio']

        # 输入: [N, C]
        N, C = last_layer_state.shape
        TOKEN_TOPK_RAW = int(N * (1 - reduction_ratio))
        ...
        if abs(TOKEN_TOPK_RAW - TOKEN_TOPK_down) <= abs(TOKEN_TOPK_RAW - TOKEN_TOPK_up):
            keep_num = TOKEN_TOPK_down
        else:
            keep_num = TOKEN_TOPK_up

        # 归一化 key
        norm_keys = F.normalize(key_state, p=2, dim=-1)  # [N, C]
        sim = torch.matmul(norm_keys, norm_keys.transpose(0, 1))  # [N, N]

        # 初始化候选索引
        cand_idx = torch.arange(N, device=last_layer_state.device)

        # 迭代丢弃，直到只剩 keep_num
        r = 16  # 和 SAINT 默认保持一致
        

        while cand_idx.shape[0] > keep_num:
            M = cand_idx.shape[0]
            cand_sim = sim[cand_idx][:, cand_idx]
            scores = cand_sim.sum(dim=-1)

            # 每轮丢掉的数量
            drop_num = min(M - keep_num, M // r)

            # 按冗余度排序，丢掉前 drop_num 个
            drop_order = scores.argsort(descending=True)
            drop_local_indices = drop_order[:drop_num]

            mask = torch.ones(M, dtype=torch.bool, device=last_layer_state.device)
        ...
        # 如果你想直接返回新的 state，可以这样：
        # retained_state = last_layer_state[cand_idx, :]
...
class DART(Qwen2VLModel):
    ...
    def forward(
        ...
        # self.config.use_saint = True
        # self.config.saint_sys_length = 36
        # self.config.saint_image_token_length = 376
        # self.model.config.saint_image_token_length = 576
        # self.config.saint_start = 7
        # self.config.saint_end = 16
        # self.config.saint_threshold = 0.83
        #self.model.set_saint()

        # SYS_LENGTH= self.saint_sys_length
        # IMAGE_TOKEN_LENGTH = self.saint_image_token_length
        use_saint = False
        SAINT_START = 7
        SAINT_THRESH = 0.83
        SAINT_END = 16
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if DART_config is not None: # and DART_config['Sparse']
                    ...
                    if use_saint and SAINT_START <= decoder_layer.self_attn.layer_idx <= SAINT_END and seq_length > 1:
                        # 取 key，做 head 平均
                        last_layer_key_values = layer_outputs['key_states']   # [B, H, N, C]
                        last_layer_key_avg = torch.mean(last_layer_key_values, dim=1)  # [B, N, C]

                        # 假设 batch=1，只取第一个样本
                        image_keys = last_layer_key_avg[0, image_token_start_index:image_token_start_index+image_token_length]

                        # 调用 saint_drop，返回索引
                        selected_indices = DART.saint_drop(image_keys.unsqueeze(0), sim_threshold=SAINT_THRESH)
                        #print("selected_indices",len(selected_indices[0]),"layer",i,"prev_length",seq_length)
                        #print("img start",image_token_start_index, image_token_length, seq_length)
                        kept_vision_indices = selected_indices[0] + image_token_start_index

                        # 系统 token 和尾部 token
                        system_indices = torch.arange(image_token_start_index, device=device)
                        tail_indices = torch.arange(image_token_start_index+image_token_length, seq_length, device=device)
                        #print("system_indices",system_indices,tail_indices)

                        # 拼接并排序
                        keep_indexs = torch.cat([system_indices, kept_vision_indices, tail_indices], dim=0)
                        keep_indexs, _ = torch.sort(keep_indexs)

                        # 直接用一维索引切 hidden_states
                        hidden_states = hidden_states[:, keep_indexs, :]
                        position_ids = position_ids[:, :, keep_indexs]
                        # attention_mask = self._prepare_decoder_attention_mask(None, (1, keep_indexs.shape[0]), hidden_states, 0)

                        # 更新长度
                        image_token_length = kept_vision_indices.shape[0]
    ...
    def saint_drop(
        ...
        with torch.no_grad():
            ...
            if r <= 0:
                indices = torch.arange(N, device=metric.device).unsqueeze(0).expand(B, -1)
                return indices
            ...
            score_alternative = scores.mean(dim=-1) 
            final_scores = torch.where(valid_counts > 0, score_candidate, score_alternative)  # shape: [B, n]
            
            sorted_indices = final_scores.argsort(dim=-1, descending=True)
            unmatched_indices = sorted_indices[..., r:]

            src_indices = unmatched_indices * 2
            indices = torch.arange(b.shape[1], device=metric.device).unsqueeze(0).expand(B, -1) * 2 + 1
            
            if src_indices.shape[1] > 0:
                indices = torch.cat([src_indices, indices], dim=1)
            #print("r",r,N,len(indices[0]))
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if (pixel_values is not None or pixel_values_videos is not None) and self.config.DART_config is not None:
                self.model.config.DART_config['image_token_length'] = int(torch.sum(retained_nums).item()) 
...
class DART_ViT(Qwen2VisionTransformerPretrainedModel):
    def __init__(self, config:Qwen2VLVisionConfig):
        ...
        self.blocks = nn.ModuleList(
            [Qwen2VLVisionBlock_Saint(config,layer_idx ,config._attn_implementation) for layer_idx in range(config.depth)]
        )
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if DART_config is not None: #and DART_config['vit_Sparse']:
                K = len(self.blocks)-1 #DART_config['vit_pruned_layer']
                
                if blk.layer_idx == K and hidden_states_pkg['hidden_states'].shape[0]>1:
                    #print("prune at layer", K,hidden_states_pkg['hidden_states'].shape[0])
            ...
            if hasattr(self, "_sparse_vit_saved"):
                ...
                hidden_states_pkg = blk(hidden_states_pkg, cu_seqlens=cu_seqlens_pruned, rotary_pos_emb=rotary_pos_emb_pruned,prune=prune)
            else:
                #print(hidden_states_pkg['hidden_states'].shape, cu_seqlens, rotary_pos_emb.shape)
                hidden_states_pkg = blk(hidden_states_pkg, cu_seqlens=cu_seqlens, rotary_pos_emb=rotary_pos_emb,prune=prune)


            if prune==True:
                #print("prune after", hidden_states_pkg['hidden_states'].shape[0])
                ...
                # sort index
                keep_indexs = hidden_states_pkg['remain_idx']
                ...
                #print("keepidx",len(keep_indexs),frame_counts)
