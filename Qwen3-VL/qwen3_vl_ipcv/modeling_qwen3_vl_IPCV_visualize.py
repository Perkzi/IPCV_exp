...
class Qwen3VLVisionBlock_Sparse(Qwen3VLVisionBlock):
    ...
    def forward(
        ...
        **kwargs,
        ...
        #print("concat states",hidden_states.shape)
        # hidden_states = hidden_states_pkg["hidden_states"]
...
class Qwen3VLVisionModel_Sparse(Qwen3VLVisionModel):
    ...
    def forward(
        ...
        for layer_num, blk in enumerate(self.blocks):
            ...
            if IPCV_config is not None and IPCV_config['vit_Sparse']:
                ...
                if layer_num == K and hidden_states_pkg['hidden_states'].shape[0]>1:
                    ...
                    if hasattr(self, 'consistency_analyzer'):
                        # 将列表转换为张量 [B, R, 10]
                        all_neighbors_tensor = rem_to_kept_idx
                        
                        self.consistency_analyzer.record_k_layer(
                            hidden_states,          # [ N, C]
                            removed_indices, # [ R]
                            keep_indexs,       # [ K]
                            all_neighbors_tensor # [R, 10]
                        )
                # 收集数据：从第 K 层到最后一层
                if hasattr(self, 'analyzer'):
                    if layer_num >= K:
                        # 这里传入 hidden_states (确保它是 ( N, C) 的原始完整状态)
                        self.analyzer.collect(layer_num, hidden_states)


            # '''修改kv的层数'''
            # if hasattr(self, "_sparse_vit_saved") and layer_num < IPCV_config['vit_pruned_layer']+7:
            #     hidden_states_pkg = blk(hidden_states_pkg, cu_seqlens=cu_seqlens, position_embeddings=position_embeddings, sparse_vit_saved=self._sparse_vit_saved, **kwargs)
            ...
            #     # 1) 拿出新旧 kept_states 与 rem_to_kept_idx
            #     new_kept        = hidden_states_pkg['hidden_states']       # [K, D]
            #     #print("new_kept_final", new_kept.shape)
            #     orig_kept       = saved["orig_kept_states"]               # [K, D]
            #     rem_to_kept_idx = saved["rem_to_kept_idx"]                # [R, 10]
            #     removed_indices = saved["removed_indices"]                # [R]
            #     unique_idx = saved["unique_idx"]
            ...
            if layer_num == len(self.blocks) - 1 and hasattr(self, 'consistency_analyzer'):
            ...
            if layer_num in self.deepstack_visual_indexes:
                ...
                #     full_states[removed_indices]    = (saved["removed_states"] + avg_delta_removed)
                # else:
                full_states = hidden_states_pkg['hidden_states']

                

                #print("deep_Stak layer",layer_num,full_states.shape)
                deepstack_feature = self.deepstack_merger_list[self.deepstack_visual_indexes.index(layer_num)](
                    full_states
                )
        ...
        #print("vit_final",hidden_states.shape)
        merged_hidden_states = self.merger(hidden_states)
    ...
    def get_retained_image_token(self, config: Qwen3VLVisionConfig, last_layer_state: torch.Tensor, any_states: torch.Tensor) -> torch.Tensor:
        ...
        # print("k_states_image_token_L1_norm",k_states_image_token_L1_norm,pivot_image_token,image_token_start_index)
    ...
    def get_retained_image_token_random(self, config: Qwen3VLVisionConfig, last_layer_state: torch.Tensor, any_states: torch.Tensor) -> torch.Tensor:
        IPCV_config = config.IPCV_config
        reduction_ratio = IPCV_config['vit_reduction_ratio']
    ...
    def get_retained_image_token_attn_scores(self, config: Qwen3VLVisionConfig, last_layer_state: torch.Tensor, any_states: torch.Tensor, attn_scores: torch.Tensor) -> torch.Tensor:
        # any_state [seq_len, num_heads, head_dim]
        IPCV_config = config.IPCV_config
        # K = IPCV_config['K']
        image_token_start_index = 0
    ...
    def get_retained_image_token_pivot_sim(self, config: Qwen3VLVisionConfig, last_layer_state: torch.Tensor, any_states: torch.Tensor) -> torch.Tensor:
        IPCV_config = config.IPCV_config
        #K = DART_config['K']
        image_token_start_index = 0
        image_token_length = last_layer_state.shape[0]
...
class TokenTrajectoryAnalyzer:
    ...
    def reset(self):
        ...
        self.neighbor_locations = None # 存储 ( local_token_idx) 的列表
        self.history = []

    def select_targets(self, removed_indices, kept_indices_all, rem_to_kept_idx_global):
        """
        removed_indices: 你的 removed_indexs
        ...
        rel_idx = torch.randint(0, len(removed_indices), (1,)).item()
        
        # 2. 锁定目标：Batch 0 中的绝对索引
        self.target_token_idx = removed_indices[rel_idx].item()
        self.target_batch_id = 0
    ...
    def collect(self, layer_idx, hidden_states):
        ...
        p_state = hidden_states[ self.target_token_idx, :].detach().cpu().float()
        
        # 提取跨 Batch 分布的 10 个邻居
        n_list = []
        for t_id in self.neighbor_locations:
            n_list.append(hidden_states[ t_id, :].detach().cpu().float())
...
class EvolutionConsistencyAnalyzer:
    ...
    def record_final_layer(self, hidden_states):
        ...
        N, C = final_states.shape
        
        # 1. 计算所有 token 的 Delta (h_final - h_k)
        delta_all = final_states - self.k_states # [ N, C]
        
        # 2. 构造全局 Kept Delta 池 (跨 Patch 索引的基础)
        # 将所有 Patch 的保留 token 拼在一起
        d_kept_all = delta_all[ self.kept_indices] 
        # d_kept_all = torch.cat(d_kept_list, dim=0) # [B*K, C]

        # 3. 统计当前图片的所有数据
        
        d_pruned = delta_all[ self.pruned_indices] # [R, C]
        ...
        for i in range(d_pruned.shape[0]):
            top10_global_idx = self.neighbor_top10_global[ i]
