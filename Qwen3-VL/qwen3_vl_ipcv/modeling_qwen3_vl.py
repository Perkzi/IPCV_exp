...
class Qwen3VLVisionModel(Qwen3VLPreTrainedModel):
    ...
    def forward(
        ...
        pos_embeds = self.fast_pos_embed_interpolate(grid_thw) # torch.Size([3672, 1152])  grid_thw tensor([[ 1, 54, 68]]
        hidden_states = hidden_states + pos_embeds # torch.Size([3672, 1152])

        rotary_pos_emb = self.rot_pos_emb(grid_thw)

        seq_len, _ = hidden_states.size()
        hidden_states = hidden_states.reshape(seq_len, -1)
        rotary_pos_emb = rotary_pos_emb.reshape(seq_len, -1)  #torch.Size([3672, 36])
        emb = torch.cat((rotary_pos_emb, rotary_pos_emb), dim=-1) # torch.Size([3672, 72])
        position_embeddings = (emb.cos(), emb.sin()) #(torch.Size([3672, 72]),torch.Size([3672, 72]))
        ...
        cu_seqlens = F.pad(cu_seqlens, (1, 0), value=0)# tensor([   0, 3672]
        ...
        for layer_num, blk in enumerate(self.blocks):
            ...
            #print("indexes",self.deepstack_visual_indexes) # [8, 16, 24]
