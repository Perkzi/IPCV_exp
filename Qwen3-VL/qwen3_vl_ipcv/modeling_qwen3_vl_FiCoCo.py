...
from qwen3_vl_ipcv.FiCoCo.FiCoCo_V_no_CLS import *
    ...
    def forward(
        ...
        **kwargs,
        ...
        if merge_visual and prune and r is not None:
            ...
            hidden_states, attn_weights, metric, att_score = adapt_qwen2vl_output(hidden_states, key_states.squeeze(0), attn_weight.squeeze(0))
        ...
        hidden_states_pkg['cu_seqlens'] = cu_seqlens
        hidden_states_pkg['remain_indices'] = remain_indices
        return hidden_states_pkg


def adapt_qwen2vl_output(hidden_states, key_states, attn_weight, bsz=1):
    """
    ...
    attn_weights_reshaped = attn_weight.unsqueeze(0).unsqueeze(0)

    # mean_keys: (seq_length, num_heads, head_dim) → 在 head 上取平均
    #print("k states",key_states.shape)
    mean_keys = key_states.mean(dim=0)  # (seq_length, head_dim)
    ...
    #print("attn score",attn_weight.shape)
    att_score = attn_weight.mean(dim=0)
...
class Qwen3VLVisionModel_Sparse(Qwen3VLVisionModel):
    ...
    def forward(
        ...
        target_keep_ratio = 1.0 - self.config.IPCV_config['vit_reduction_ratio']  # 0.2
        ...
        if self.config.IPCV_config is not None and self.config.IPCV_config['vit_Sparse']\
            and not self.update_attention_layer:
            K = IPCV_config['vit_pruned_layer']
        ...
        for layer_num, blk in enumerate(self.blocks):
            ...
            if IPCV_config is not None and IPCV_config['vit_Sparse']:
                K = IPCV_config['vit_pruned_layer']
                ...
                # if K-1>0 and layer_num ==K-1 and IPCV_config['vit_diff_choose'] and hidden_states_pkg['hidden_states'].shape[0]>1:
                ...
                if layer_num >= K and hidden_states_pkg['hidden_states'].shape[0]>1 and\
                    ...
                    hidden_states_pkg = blk(hidden_states_pkg, cu_seqlens=cu_seqlens, position_embeddings=position_embeddings, prune=prune, merge_visual=merge_visual,grid_thw=grid_thw, r=tome_r, **kwargs)

                    # 更新当前保留比例
                    current_keep_ratio *= desired_keep_ratio
                    # 更新位置编码
                    #rotary_pos_emb = rotary_pos_emb[hidden_states_pkg['merged_idx'],:][0]
                    position_embeddings = (position_embeddings[0][hidden_states_pkg['remain_indices'],:], position_embeddings[1][hidden_states_pkg['remain_indices'],:])
                    ...
                    prune = False
                    hidden_states_pkg = blk(hidden_states_pkg, cu_seqlens=cu_seqlens, position_embeddings=position_embeddings, prune=prune, merge_visual=merge_visual,grid_thw=grid_thw, **kwargs)
            else:
                prune = False
                hidden_states_pkg = blk(hidden_states_pkg, cu_seqlens=cu_seqlens, position_embeddings=position_embeddings, prune=prune, merge_visual=merge_visual,grid_thw=grid_thw, **kwargs)
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            # IPCV_config = self.config.IPCV_config
            # if IPCV_config is not None and IPCV_config['Sparse']:
            ...
            #         # keep index
            #         #print("image_token_length",image_token_length)#,image_token_start_index)
