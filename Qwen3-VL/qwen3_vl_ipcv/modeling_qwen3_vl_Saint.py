...
class Qwen3VLVisionAttention_Sparse(Qwen3VLVisionAttention):
    def __init__(self, config: Qwen3VLVisionConfig) -> None:
    ...
    def forward(
        ...
        **kwargs,
        ...
        if is_flash_attention_requested(self.config):
            ...
            # 分别取出输出和权重
            attn_outputs = [res[0] for res in attn_results]

            # 拼接输出
            attn_output = torch.cat(attn_outputs, dim=1)

            if attn_results[0][1] is not None:
                attn_weights = [res[1] for res in attn_results]
                # 如果需要拼接权重，也可以：
                attn_weight = torch.cat(attn_weights, dim=1)
            else:
                attn_weight = None
        ...
        return attn_output, key_states, attn_weight
...
class Qwen3VLVisionBlock_Sparse(Qwen3VLVisionBlock):
    def __init__(self, config, attn_implementation: str = "sdpa") -> None:
        super().__init__(config, attn_implementation)
        self.attn = Qwen3VLVisionAttention_Sparse(config=config)
        self.config = config
        

    def forward(
        self,
        hidden_states_pkg: torch.Tensor,
        cu_seqlens: torch.Tensor,
        rotary_pos_emb: torch.Tensor | None = None,
        position_embeddings: tuple[torch.Tensor, torch.Tensor] | None = None,
        prune=False,
        **kwargs,
        ...
        attn_output, key_states, attn_weight = self.attn(
            self.norm1(hidden_states),
            cu_seqlens=cu_seqlens,
            rotary_pos_emb=rotary_pos_emb,
            position_embeddings=position_embeddings,
        ...
        if prune:
            remain_idx = self.iterative_drop_full_graph(self.config, hidden_states, key_states.squeeze(0).permute(1, 0, 2).mean(1)) #  torch.Size([5220, 1280]) torch.Size([5220, 16, 80])
        ...
        hidden_states_pkg["hidden_states"] = hidden_states
        hidden_states_pkg["k_states"] = key_states.squeeze(0).permute(1, 0, 2)
        if attn_weight is not None:
            hidden_states_pkg["attn_scores"] = attn_weight.squeeze(0)
        hidden_states_pkg['remain_idx'] = remain_idx
        # print("block output",hidden_states.shape,hidden_states_pkg["k_states"].shape)
        # if attn_weight is not None:
        #     print(attn_weight.shape)
        return hidden_states_pkg

    def iterative_drop_full_graph(self, config, last_layer_state: torch.Tensor, key_state: torch.Tensor):
        #print("last_layer_state.shape",last_layer_state.shape,key_state.shape,config.IPCV_config)
        # last_layer_state = last_layer_states[0]
        # key_state = key_states[0]

        IPCV_config = config.IPCV_config
        reduction_ratio = IPCV_config['vit_reduction_ratio']
...
class Qwen3VLTextAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        return attn_output, key_states, attn_weights
...
class Qwen3VLTextDecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # Self Attention
        hidden_states, key_states, attn_scores = self.self_attn(
...
class Qwen3VLVisionModel_Sparse(Qwen3VLVisionModel):
    config: Qwen3VLVisionConfig
    _no_split_modules = ["Qwen3VLVisionBlock"]
    _can_record_outputs = {
        "hidden_states": Qwen3VLVisionBlock,
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        self.blocks = nn.ModuleList([Qwen3VLVisionBlock_Sparse(config) for _ in range(config.depth)])
    ...
    def forward(
        ...
        #--------------
        deepstack_feature_lists_raw = []
        deepstack_feature_lists = []
        for layer_num, blk in enumerate(self.blocks):
            IPCV_config = self.config.IPCV_config
            if IPCV_config is not None: #and DART_config['vit_Sparse']:
                K = len(self.blocks)-1 #DART_config['vit_pruned_layer']
                
                if layer_num == K and hidden_states_pkg['hidden_states'].shape[0]>1:
            ...
            if hasattr(self, "_sparse_vit_saved"):
                ...
                hidden_states_pkg = blk(hidden_states_pkg, cu_seqlens=cu_seqlens_pruned, position_embeddings=position_embeddings_pruned,prune=prune, **kwargs)
            else:
                #print(hidden_states_pkg['hidden_states'].shape, cu_seqlens, rotary_pos_emb.shape)
                hidden_states_pkg = blk(hidden_states_pkg, cu_seqlens=cu_seqlens, position_embeddings=position_embeddings,prune=prune, **kwargs)
            ...
            if prune==True:
                ...
                position_embeddings_pruned = (position_embeddings[0][keep_indexs,:], position_embeddings[1][keep_indexs,:])
                ...
                #print("keep_indexs",keep_indexs.shape,cu_seqlens)
            ...
            if layer_num in self.deepstack_visual_indexes:
                deepstack_feature_lists_raw.append(hidden_states)
                #print("deep_Stak layer",layer_num,hidden_states.shape)
        for i, states in enumerate(deepstack_feature_lists_raw):
            #print("deepstack",keep_indexs,states.shape,states[keep_indexs,:].shape)
            deepstack_feature = self.deepstack_merger_list[i](
                states[keep_indexs,:]
            )
    ...
    def update_vision_block(self, device, dtype):
        # 获取需要替换的层索引
        k = self.config.IPCV_config['vit_pruned_layer'] - 1
        if k<0 : return 
        # 旧 block
        old_block = self.blocks[k]

        new_config = copy.deepcopy(self.config) 
        new_config._attn_implementation = 'eager' # self.config._attn_implementation
        # 创建新 block，使用 'eager' 注意力实现
        new_block = Qwen3VLVisionBlock_Sparse(
            new_config,
            #layer_idx=k,
...
class Qwen3VLTextModel_Sparse(Qwen3VLTextModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        #print("attn mask",attention_mask,"pastkv",past_key_values)

        # 假设 past_key_values 是 DynamicCache
        # for i in range(2, 6):  # 层索引 2 到 5
        #     layer = past_key_values.layers[i]
        #     k = layer.keys   # 不同实现可能是 layer.kv_cache 或类似属性
        #     if k is not None:
        #         print(f"Layer {i}: key {k.shape}")
        ...
        IPCV_config = self.config.IPCV_config
        if IPCV_config is not None:
            #K = IPCV_config['pruned_layer']  # pruned layer
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if IPCV_config is not None:
                ...
                if use_saint and SAINT_START <= decoder_layer.self_attn.layer_idx <= SAINT_END and seq_length > 1:
                    ...
                    # 调用 saint_drop，返回索引
                    selected_indices = Qwen3VLTextModel_Sparse.saint_drop(image_keys.unsqueeze(0), sim_threshold=SAINT_THRESH)
                    ...
                    #print("pos emb",position_embeddings[0].shape,len(position_embeddings)) # pos emb torch.Size([1, 721, 128]) 2
    ...
    def _deepstack_process(
        ...
        visual_embeds = visual_embeds.to(hidden_states.device, hidden_states.dtype)
        hidden_states = hidden_states.clone()
        # print("deepstack",visual_pos_masks.shape,visual_embeds.shape,hidden_states.shape)
        # print(visual_pos_masks)
        local_this = hidden_states[visual_pos_masks, :] + visual_embeds
        hidden_states[visual_pos_masks, :] = local_this
        # print("shape after deepstack",hidden_states.shape)
        return hidden_states
    ...
    def update_layer(self, device=None, dtype=None):
        # 获取需要替换的层索引
        k = self.config.IPCV_config['pruned_layer'] - 1
        if k<0 : return 
        # 旧 block
        ...
        new_config = copy.deepcopy(self.config) 
        new_config._attn_implementation = 'eager' # self.config._attn_implementation
        new_block = Qwen3VLTextDecoderLayer(
            new_config,
            layer_idx=k,
            # attn_implementation='eager'
        )
...
class Qwen3VLModel_Sparse(Qwen3VLModel):
    ...
    def get_image_features(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        pixel_values = pixel_values.type(self.visual.dtype)
        vision_output, frame_counts = self.visual(
            pixel_values, grid_thw=image_grid_thw, return_dict=True, **kwargs
        )
        image_embeds = vision_output.pooler_output
        if frame_counts is not None: #self.visual.config.IPCV_config is not None and self.visual.config.IPCV_config['vit_Sparse']:
            split_sizes = frame_counts
        ...
        #print("embeds",image_embeds[0].shape,len(image_embeds))

        return vision_output, split_sizes
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if pixel_values is not None:# and self.config.IPCV_config['vit_Sparse']:
            ...
            #print("image_embeds.shape",image_embeds.shape, len(deepstack_image_embeds),input_ids.shape) # torch.Size([77, 4096]) 3 torch.Size([1, 721])

            input_ids_new, retained_indices = Qwen3VLModel_Sparse._update_ids(input_ids, self.config.image_token_id, split_sizes)

            #image_mask = input_ids_new == self.config.image_token_id

            inputs_embeds = inputs_embeds[:,retained_indices,:]

            #print("inputs_embeds",inputs_embeds,inputs_embeds.shape,position_ids.shape,attention_mask.shape) 
            ...
            image_mask, _ = self.get_placeholder_mask(
                input_ids_new, inputs_embeds=inputs_embeds, image_features=image_embeds
            )
            inputs_embeds = inputs_embeds.masked_scatter(image_mask, image_embeds)
            #print("inputs_embeds2",inputs_embeds,inputs_embeds.shape,position_ids.shape,attention_mask.shape,image_mask.shape)
            #  torch.Size([1, 582, 4096]) torch.Size([4, 1, 582]) torch.Size([1, 582]) torch.Size([1, 582, 4096]

            cache_position = cache_position[retained_indices] # TODO
        ...
        if image_mask is not None and video_mask is not None:
            ...
            #print("image_mask",image_mask.shape,image_mask.sum().item())
            image_mask = image_mask[..., 0]
            visual_pos_masks = image_mask
            deepstack_visual_embeds = deepstack_image_embeds

            # print("visual_pos_masks True count:", visual_pos_masks.shape, visual_pos_masks.sum().item())

            # for i, embeds in enumerate(deepstack_visual_embeds):
            #     print(f"Layer {i}: {embeds.shape}")
            #     # if embeds.shape[0]>visual_pos_masks[1]:
            #     #     embeds = 
