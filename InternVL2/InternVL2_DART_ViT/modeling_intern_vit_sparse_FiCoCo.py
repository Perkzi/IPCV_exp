...
from InternVL2_DART_ViT.FiCoCo.FiCoCo_V import *
...
class InternVisionEncoderLayer(nn.Module):
    ...
    def forward(
            ...
            prune=False, merge_visual=False,initial_len=None, r=None
    ) -> Tuple[torch.FloatTensor, Optional[torch.FloatTensor], Optional[Tuple[torch.FloatTensor]]]:
        """
        Args:
            hidden_states (`Tuple[torch.FloatTensor, Optional[torch.FloatTensor]]`): input to the layer of shape `(batch, seq_len, embed_dim)`
        """
        #print("hidden0",hidden_states.shape)
        ...
        if merge_visual and prune and r is not None:
            for b in range(B):
                ...
                hid_state, attn_weights, metric, att_score = adapt_qwen2vl_output(hidden_states[b], attn_outputs['k_states'][b], attn_outputs['attn_scores'][b])
                ...
                if rr > 0:
                    ...
                    order_indices, merge_indices, remain_indices, att_prob = Filter(
                        att_score, metric, rr, include_class_token=False,initial_len=initial_len)                     # qwen没有class token   _ficoco_info["class_token"]
                    #print("order_indices1",order_indices.shape, merge_indices.shape, remain_indices.shape, att_prob.shape)
                    # torch.Size([1, 1025, 1]) torch.Size([1, 204, 1]) torch.Size([1, 821, 1]) torch.Size([1, 1025, 1025])
                    # Correlate 
                    target_indices, top_values = Correlate(merge_indices, att_prob)
                    
                    #print("target_indices",target_indices.shape, top_values.shape,hidden_states.shape)
                    # torch.Size([1, 204, 3]) torch.Size([1, 1025, 3]) torch.Size([3, 1025, 3200])
                    ...
                    hid_state, _ = merge_ficoco_v(
                        Compress, hid_state, merge_indices, remain_indices, top_values, target_indices, rr, None) #  _ficoco_info["size"]
                    ...
                    hid_state = hid_state[0]

                    all_hid_states.append(hid_state)
                    all_remain_indices.append(remain_indices) # 包括cls token?
            ...
            hidden_states = torch.stack(all_hid_states, dim=0)
            all_remain_indices = torch.stack(all_remain_indices, dim=0)


        hidden_states = hidden_states + self.drop_path2(self.mlp(self.norm2(hidden_states).to(hidden_states.dtype)) * self.ls2)

        #print("hidden1",hidden_states.shape)
        #return hidden_states
    
        return {'hidden_states':hidden_states,
                'attn_scores':attn_outputs['attn_scores'],
                'k_states':attn_outputs['k_states'],
                'all_remain_indices': all_remain_indices
                }
    


def adapt_qwen2vl_output(hidden_states, k_states,attn_scores, bsz=1):
    """
    ...
    attn_weights_reshaped = attn_scores.unsqueeze(0).unsqueeze(0)

    # mean_keys: (seq_length, num_heads, head_dim) → 在 head 上取平均
    #print("k states",k_states.shape)
    mean_keys = k_states.mean(dim=0)  # (seq_length, head_dim)
    ...
    att_score = attn_scores.mean(dim=0)
...
class InternVisionEncoder_Sparse(InternVisionEncoder):
    ...
    def forward(
        ...
        initial_len = hidden_states.shape[1]
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if DART_config is not None and DART_config['vit_Sparse']:
                    ...
                    if idx >= K and seq_len > 1 and\
                        ...
                        hidden_states_pkg = blk(hidden_states, prune=prune, merge_visual=merge_visual,initial_len=initial_len, r=tome_r)
                        hidden_states = hidden_states_pkg['hidden_states']
                        ...
                        prune = False
                        hidden_states_pkg = blk(hidden_states, prune=prune, merge_visual=merge_visual,initial_len=initial_len)
                        hidden_states = hidden_states_pkg['hidden_states']
                else:
                    prune = False
                    hidden_states_pkg = blk(hidden_states, prune=prune, merge_visual=merge_visual,initial_len=initial_len)
