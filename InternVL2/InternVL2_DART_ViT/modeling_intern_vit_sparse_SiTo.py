...
class InternVisionEncoder_Sparse(InternVisionEncoder):
                    ...
                    if idx == K and seq_len > 1:
                        ...
                        for b in range(B):
                            b_idx, a_unm_idx, a_src_idx, max_indices = self.vlm_sito_prune(
                                                self.config, last_layer_state[b,1:])
                             # 保留的索引 = 基准 + 未剪枝 
                            keep_idx = torch.cat([b_idx, a_unm_idx]).sort().values
    ...
    def vlm_sito_prune(self, config, hidden_states: torch.Tensor, 
        ...
        # 按相似度排序，选出要剪掉的 token
        num_remain =  int(seq_len * (1-reduction_ratio)) 
        
        num_remain = (int(num_remain**0.5) // 2 * 2) ** 2 # 偶数的平方
        
        num_prune = seq_len- num_remain #int(seq_len * reduction_ratio) 
