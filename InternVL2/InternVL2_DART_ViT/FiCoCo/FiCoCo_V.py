...
def Filter(
    ...
    current_token_num  = att_score.shape[1]   # 或者 att_score.shape[1]

    global dst_score_global_dict
    dst_score_global = None
    if (batch_idx in dst_score_global_dict):
        dst_score_global = dst_score_global_dict[batch_idx]
    # 初始化或检查全局缓存
    if (batch_idx not in dst_score_global_dict) or dst_score_global is None\
          or dst_score_global.shape[1] != initial_len-1 or current_token_num == initial_len-1:
        
        # 初次进入，全置为极大值 (1e9)
        dst_score_global = torch.full((1, initial_len-1), 65504.0, 
                                     device=att_score.device, dtype=att_score.dtype)
        dst_score_global_dict[batch_idx] = dst_score_global
