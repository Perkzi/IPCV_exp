...
class InternVisionEncoderLayer_Sparse(InternVisionEncoderLayer):
    def __init__(self, config: InternVisionConfig, drop_path_rate: float):
        self.config = config
    ...
    def forward(
        ...
        #print("before prune",hidden_states.shape,attn_outputs['k_states'].shape)
       
        if prune:
            remain_indices = []
            pruned_hidden_states = []
            for b in range(hidden_states.shape[0]):  # 遍历 batch
                # 分离 CLS 和 patch tokens
                cls_token = hidden_states[b:b+1, 0:1, :]          # [1, 1, C]
                patch_tokens = hidden_states[b, 1:, :]            # [N-1, C]
                key_tokens = attn_outputs['k_states'][b, 1:, :]   # [N-1, D]

                # 剪枝 patch tokens
                remain_idx_b = self.iterative_drop_full_graph(
                    self.config,
                    patch_tokens,
                    key_tokens.mean(1)
                )
                remain_indices.append(remain_idx_b)
                pruned = patch_tokens[remain_idx_b]

                # 拼回 CLS + 剪枝后的 patch
                pruned_hidden_states.append(torch.cat([cls_token.squeeze(0), pruned], dim=0))

            hidden_states = torch.stack(pruned_hidden_states, dim=0)
            #print("after prune",hidden_states.shape,remain_indices[0].shape)
        ...
        # if prune:
        #     remain_idx = self.iterative_drop_full_graph(self.config, hidden_states, attn_outputs['k_states'].mean(1)) #  torch.Size([5220, 1280]) torch.Size([5220, 16, 80])
        ...
        return {'hidden_states':hidden_states,
                'attn_scores':attn_outputs['attn_scores'],
                'k_states':attn_outputs['k_states'],
                'remain_idx':remain_indices}
    

    def iterative_drop_full_graph(self, config, last_layer_state: torch.Tensor, key_state: torch.Tensor):
        #print("last_layer_state.shape",last_layer_state.shape,key_state.shape) # torch.Size([ 1025, 1024]) torch.Size([ 16, 64])
        ...
        # 如果平方根是奇数，就减 1，保证是偶数 
        if max_square_root % 2 == 1: 
            max_square_root -= 1
        keep_num = max_square_root ** 2  # 得到平方数
...
class InternVisionEncoder_Sparse(InternVisionEncoder):
    ...
    def __init__(self, config: InternVisionConfig):
        ...
        dpr = [x.item() for x in torch.linspace(0, config.drop_path_rate, config.num_hidden_layers)]
        self.layers = nn.ModuleList([
            InternVisionEncoderLayer_Sparse(config, dpr[idx]) for idx in range(config.num_hidden_layers)])
        ...
        for idx, blk in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if DART_config is not None: #and DART_config['vit_Sparse']:
                    K = self.config.num_hidden_layers-1 #DART_config['vit_pruned_layer']
                ...
                #     hidden_states_pkg = blk(hidden_states_pkg, prune=prune)
                # else:
                #     #print(hidden_states_pkg['hidden_states'].shape, cu_seqlens, rotary_pos_emb.shape)
                hidden_states_pkg = blk(hidden_states_pkg['hidden_states'], prune=prune)
